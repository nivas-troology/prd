...
@Component({
  selector: 'app-punishment-report',
  templateUrl: './punishment-report.component.html',
  styleUrls: ['./punishment-report.component.css']
})
export class PunishmentReportComponent implements OnInit {
 ...
 page:number=1
  ...
  ngOnInit(): void {
   ...
   if(this.MRoleId==1)
   {
    ...
    this.spinner.show();
      this.reportservice.getPunishmentJawan(0,0,0,'~').subscribe((data:any)=>{this.PunishJawanlist=data;
        this.spinner.hide();
   ...
   this.disabledrpDivision=true;
    //this.getPunishmentJawan(parseInt(this.ZoneId),this.DistId,0);
   ...
   else if(this.MRoleId==3)
   {
    ...
    //this.getPunishmentJawan(parseInt(this.ZoneId),parseInt(this.DistId),0);
    this.disabledrpDivision=true;
...
ClickUrbanRuralCurrent()
{
  ...
  // Get All Jawans
  PunishJawanlist:any;
  getPunishmentJawan(ZoneId,DistId,CompanyId,residanceTypeCurrent)
  { 
    if(ZoneId=="--Select--")
    {
      ZoneId=0;
    ...
    residanceTypeCurrent=residanceTypeCurrent;
      this.spinner.show();
      this.reportservice.getPunishmentJawan(ZoneId,DistId,CompanyId,residanceTypeCurrent).subscribe((data:any)=>{this.PunishJawanlist=data;
        this.spinner.hide();
        this.rerender();
